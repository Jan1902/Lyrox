...
lyroxClient.ChatMessageReceived += async (_, e) => await HandleChatMessage(e.Message);

Console.ReadLine();

async Task HandleChatMessage(string message)
{
    ...
    await lyroxClient.SendChatMessage("Aye sir!");

    if (tokens[1] == "goto")
        lyroxClient.Goto(new Vector3d(double.Parse(tokens[2]), double.Parse(tokens[3]), double.Parse(tokens[4])));

    else if (tokens[1] == "block")
        await lyroxClient.SendChatMessage(lyroxClient.GetBlock(new Vector3i(int.Parse(tokens[2]), int.Parse(tokens[3]), int.Parse(tokens[4])))?.BlockName ?? "UNKNOWN");
    ...
    else if (tokens[1] == "chunk")
    {
        ...
        await lyroxClient.SendChatMessage(text);
    }
}
